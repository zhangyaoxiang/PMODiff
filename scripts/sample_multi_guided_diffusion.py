    ...
    for i in tqdm(range(num_batch)):
        ...
        with torch.no_grad():
            ...
            if sample_num_atoms == 'prior':
                pocket_size = atom_num.get_space_size(batch.protein_pos.detach().cpu().numpy()) #计算蛋白质口袋大小
            ...
            if pos_only:
                ...
                uniform_logits = torch.zeros(len(batch_ligand), model.num_classes).to(device) #[N,13]
...
if __name__ == '__main__':
    ...
    pred_pos, pred_v, pred_pos_traj, pred_v_traj, pred_v0_traj, pred_vt_traj, pred_pos0_traj, time_list  = sample_guided_diffusion_ligand(
        ...
        data=data,
        num_samples=config.sample.num_samples, #100
        
        batch_size=args.batch_size, 
        device=args.device,
        num_steps=config.sample.num_steps,#1000
        pos_only=config.sample.pos_only,#False
        center_pos_mode=config.sample.center_pos_mode,#protein
        sample_num_atoms=config.sample.sample_num_atoms#prior
    )
    result = {
        'data': data,
