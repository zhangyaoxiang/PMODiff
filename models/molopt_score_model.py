    ...
    def sample_multi_guided_diffusion(self, guide_models, guide_configs, n_data, device, protein_pos, protein_v, batch_protein,
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            for guide_model, guide_config in zip(guide_models, guide_configs):
                guide_weight = guide_config.weight #0.33
                gradient_scale_cord = guide_config.gradient_scale_cord #20
                gradient_scale_categ = guide_config.gradient_scale_categ #0
                clamp_pred_min = guide_config.get("clamp_pred_min", None) #None
                clamp_pred_max = guide_config.get("clamp_pred_max", None) #1.0

                kind = guide_config.guide_kind#kd
