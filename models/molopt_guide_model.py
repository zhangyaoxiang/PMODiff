...
class DockGuideNet3D(nn.Module):
    ...
    def forward(self, protein_pos, protein_atom_feature, ligand_pos, ligand_atom_feature, batch_protein, batch_ligand,
        ...
        if self.time_emb_dim > 0:
            if self.time_emb_mode == 'simple':
                ...
                time_feat = self.time_emb(time_step)#[batch,4]
                time_feat = time_feat[batch_ligand] #[M,4]
                input_ligand_feat = torch.cat([ligand_atom_feature, time_feat], -1) #[M,17]
            else:
                raise NotImplementedError
        else:
            input_ligand_feat = ligand_atom_feature

        init_ligand_h = self.ligand_atom_emb(input_ligand_feat) #[M,127]
        if self.config.node_indicator:
            init_ligand_h = torch.cat([init_ligand_h, torch.ones(len(init_ligand_h), 1).to(init_ligand_h.device)], -1)  #[M,128]
        ...
        if self.drop_protein_in_guide is True:
            ...
            h_protein = self.protein_atom_emb(protein_atom_feature)  #[N,127]
            
            if self.config.node_indicator:
                h_protein = torch.cat([h_protein, torch.zeros(len(h_protein), 1).to(h_protein.device)], -1)  #[N,128]
    ...
    def get_loss(
    ...
    ):   #protein [N,3]   [N,27]  [N]  ligand [N,3] [N] [N]  dock 该复合物的BA 或者 QED 或者 SA
        ...
        preds = self(
            protein_pos=protein_pos, #[N,3]
            protein_atom_feature=protein_v, #[N,27]
            batch_protein=batch_protein, #[N]

            ligand_pos=ligand_pos_perturbed, #[M,3]
            ligand_atom_feature=F.one_hot(ligand_v_perturbed, self.num_classes),#[M,13]
            batch_ligand=batch_ligand,#[M]
            time_step=time_step, #[batch]
            fix_x=True
        )
